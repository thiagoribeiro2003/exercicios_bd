...
<!-- PAREI AQUI -->
```sql
SELECT (cursos.titulo) AS "Cursos",COUNT(alunos.curso_id) AS "Qtd. de alunos por curso"  FROM alunos INNER JOIN cursos
...
```sql 
SELECT alunos.nome, alunos.primeira_nota, alunos.segunda_nota, ROUND((primeira_nota + segunda_nota)/2) AS 'Média' FROM alunos;
```
#
